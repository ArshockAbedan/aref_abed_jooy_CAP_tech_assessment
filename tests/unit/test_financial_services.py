...
def test_calc_customers_balance(my_withdrawals, my_deposits):
    ...
    assert output == financial_services.calc_customers_balance(withdrawals=my_withdrawals,
...
def test_get_total_amount_for_spender_per_category(my_withdrawals):
    ...
    assert output == financial_services.get_total_amount_for_spender_per_category(withdrawals=my_withdrawals,
                                                                                  needed_category=category,
                                                                                  spender=customer)


def test_get_unique_customers_by_category(my_withdrawals):
    category = 'Debit Payment'
    output = {'Glen M. Skipper', 'Abhay Ganymede'}
    assert output == financial_services.get_unique_customers_by_category(withdrawals=my_withdrawals,
                                                                         needed_category=category)


def test_get_unique_customers(my_withdrawals):
    output = {'Abhay Ganymede', 'Glen M. Skipper', 'Sophie Olga', 'Michael P. Taylor'}
    assert output == financial_services.get_unique_customers(withdrawals=my_withdrawals)


def test_get_unique_categories(my_withdrawals):
    output = {'Debit Payment', 'Bill', 'ATM'}
    assert output == financial_services.get_unique_categories(withdrawals=my_withdrawals)
